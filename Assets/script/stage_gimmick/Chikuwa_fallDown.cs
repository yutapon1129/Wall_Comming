...
public class Chikuwa_fallDown : MonoBehaviour
{
    [Header("スプライトがあるオブジェクト")] public GameObject spriteObj;
    [Header("プレイヤーの判定をするスクリプト")] public Chikuwa_trigger trigger;
    ...
    private void Start()
    {
        ...
        if (spriteObj != null && trigger != null && col != null && rb != null)
        {
            ...
            if (sr == null)
            {
                Debug.Log("fallDownFloor インスペクターに設定し忘れがあります1");
                Destroy(this);
            }
        }
        else
        {
            ...
            Debug.Log(spriteDefaultPos);
            Debug.Log(fallVelocity);
            Debug.Log(floorDefaultPos);
            Debug.Log(sr);
            Destroy(this);
        }
    }
